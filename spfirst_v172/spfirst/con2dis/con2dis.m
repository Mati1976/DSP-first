...
%globals
Ver = 'CON2DIS ver. 2.31';  % Version string for figure title
...
switch action
    ...
    case 'Initialize'
        ...
        set(gcf,'Name',Ver,'visible','off');
        h.FigPos = get(gcf,'Position');
        ...
        guidata(gcf,h);
        set(gcf,'HandleVisibility','callback','visible','on'); % Make figure inaccessible from command line
    ...
    case {'Editbox2','Slider2'}
        ...
        if h.wflag == 1   
            ...
            if h.MATLABVER>=8.4
                set(h.TextFs,'String',['$$\bf\omega_s = ' sprintf('%.1f',pi2*FsN) '$$ (rad/s)'],'Interpreter','Latex');
            else
    ...
    case 'Editbox3'
        ...
        set(h.TextPh,'String',['\bf Phase = $$' sprintf('%.2f',h.phaseN) '$$']);
        guidata(gcbf,h);
        Fochange(h,FoN,FsN,h.time1);
        %====================================================================
    case 'SetLineWidth'
    ...
    case 'Unfold'
        ...
        set([h.Hide';h.Line6';h.Line6c'],'visible','on');   
        %====================================================================
    case 'Foldin'
        %====================================================================   
        set(gcbo,'Checked','off');
    ...
    case 'ShowW' 
        ...
        if strcmp(show,'off')
            ...
            if h.MATLABVER>=8.4
                set(get(h.Axis4,'xlabel'),'string','\bf $$\omega$$ (rad/s)');
            else
                set(get(h.Axis4,'xlabel'),'string','\omega (rad/s)');
            end
            
            set(gcbf,'CurrentAxes',h.Axis6);            
            if h.MATLABVER>=8.4
                set(get(h.Axis6,'xlabel'),'string','\bf $$\omega$$ (rad/s)');
            else
                set(get(h.Axis6,'xlabel'),'string','\omega (rad/s)');
            end
            ...
            if h.MATLABVER>=8.4
                set(h.TextFo,'String',['\bf $$\omega_o = ' sprintf('%.1f',WoN) '$$ (rad/s)'],'Interpreter','Latex');
                set(h.TextFs,'String',['\bf $$\omega_s = ' sprintf('%.1f',WsN) '$$ (rad/s)'],'Interpreter','Latex');
            else
            ...
            set(gcbf,'CurrentAxes',h.Axis4);
            if h.MATLABVER>=8.4
                set(get(h.Axis4,'xlabel'),'string','\bf $$f$$ (Hz)');
            else
                set(get(h.Axis4,'xlabel'),'string','f (Hz)');
            end
            set(gcbf,'CurrentAxes',h.Axis6);
            if h.MATLABVER>=8.4
                set(get(h.Axis6,'xlabel'),'string','\bf $$f$$ (Hz)');
            else
                set(get(h.Axis6,'xlabel'),'string','f (Hz)');
            end
            ...
            if h.MATLABVER>=8.4
                set(h.TextFo,'String',['\bf $$f_o = ' sprintf('%.1f',FoN) '$$ (Hz)'],'Interpreter','Latex');
                set(h.TextFs,'String',['\bf $$f_s = ' sprintf('%.1f',FsN) '$$ (Hz)'],'Interpreter','Latex');
            else
    ...
    case 'ShowW_hat'
        ...
        FontSizeLabel = get(get(h.Axis3,'xlabel'),'FontSize');        
        if strcmp(show_hat,'off')
            ...
            if h.MATLABVER>=8.4
               set(get(h.Axis3,'xlabel'),'string','\bf $$\hat\omega = 2\pi\left({f_o}/{f_s}\right)$$','color','b','FontSize',FontSizeLabel);
               set(h.Axis3,'XTickLabel',{'-2\pi';'-\pi';'0';'\pi';'2\pi'});
            else
               set(get(h.Axis3,'xlabel'),'string','\omega = 2\pi (f_o / f_s)','color','b','FontSize',FontSizeLabel);
            ...
            if h.MATLABVER>=8.4
                set(get(h.Axis3,'xlabel'),'string','\bf $$f={f_o}/{f_s}$$');
                set(get(h.Axis3,'xlabel'),'fontunits','points','fontsize',h.FontSizeLabel);
                set(get(h.Axis3,'xlabel'),'fontunits','normalized');
            else
                set(h.Axis3,'FontName','Helvetica');
                set(get(h.Axis3,'xlabel'),'string','f = f_o / f_s');
            end
